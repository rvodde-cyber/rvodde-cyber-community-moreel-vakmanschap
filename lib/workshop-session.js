...
export function getSessionSecret() {
  return process.env.WORKSHOP_SESSION_SECRET || "dev-only-change-in-production";
}
...
export async function createSessionToken(workshopNaam, expiresAtMs) {
  const payload = JSON.stringify({ exp: expiresAtMs, w: workshopNaam });
...
export async function verifySessionToken(token) {
  ...
  const expected = await sign(payloadB64, getSessionSecret());
